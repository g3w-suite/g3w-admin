 ...
 */
if (!customElements.get('g3w-dialog')) {
  customElements.define('g3w-dialog', class extends HTMLElement {
    ...
    connectedCallback() {
      ...
      .close                    { cursor: pointer; border: none; padding: 5px 10px; text-transform: lowercase; letter-spacing: 1px;}
      ::backdrop                { backdrop-filter: blur(3px); }
      dialog.spin               { display: none; }
      .modal-backdrop           { position: fixed; inset: 0; cursor: pointer; }
      </style>
      <slot name="modal-button" onclick="this.nextElementSibling.showPopover()" style="cursor: pointer;">Open modal</slot>
      <dialog popover>
        <slot name="modal-header">
          <button class="close" style="float: right;" title="${ globalThis.gettext('Close') }">&times;</button>
          <slot name="modal-title"><h2>${ this.getAttribute('modal-title') }</h2></slot>
        </slot>
        <slot name="modal-body"></slot>
        <slot name="modal-footer">
          <button class="close">&times; ${ globalThis.gettext('Close') }</button>
        </slot>
      </dialog>
      <div class="modal-backdrop" hidden></div>`;
      const p = this.shadowRoot.querySelector('dialog');
      const slot = this.shadowRoot.querySelector('slot[name="modal-button"]');
      this.#button = this.#button || slot?.assignedElements()[0];
      // prevent breaking some CSS rules in edge cases (move "modal-button" immediately before <g3w-dialog> element)
      if (this.#button) {
      ...
      p.addEventListener('beforetoggle', async e => {
        try {
          if ('closed' == e.newState) {
            return b.setAttribute('hidden', '');
          }
          b.removeAttribute('hidden');
          p.classList.add('spin');
          // dynamically retrieve dialog content from <g3w-dialog url="...">
          const url = this.getAttribute('url');
          if (url) {
            p.querySelector('slot[name="modal-body"]').innerHTML = (await (await fetch(url)).text());
          }
        } catch (e) {
