...
class ProjectSerializer(G3WRequestSerializer, serializers.ModelSerializer):
    ...
    def _set_options(self, instance):
        ...
        if (
            hasattr(instance, "multilayer_query")
            and instance.multilayer_query == "multiple"
        ):
            options["querymultilayers"].append("query")

        if (
            hasattr(instance, "multilayer_querybybbox")
            and instance.multilayer_querybybbox == "multiple"
        ):
            options["querymultilayers"].append("querybbox")

        if (
            hasattr(instance, "multilayer_querybypolygon")
            and instance.multilayer_querybypolygon == "multiple"
        ):
        ...
        # legend position
        options["legend_position"] = getattr(instance, "legend_position", "tab")
    ...
    def get_metadata(self, instance, qgs_project):
        ...
        for service_property in [
            ...
            metadata[service_property.lower()] = getattr(
                QgsServerProjectUtils, f"owsService{service_property}"
            )(qgs_project)
        ...
        for property in ("ContactPerson", "ContactOrganization", "ContactPosition"):
            metadata["contactinformation"]["personprimary"].update(
                {
                    property: getattr(QgsServerProjectUtils, f"owsService{property}")(
                        qgs_project
                    )
                }
            )

        metadata["contactinformation"].update(
            OrderedDict(
                {
                    "contactvoicetelephone": QgsServerProjectUtils.owsServiceContactPhone(
                        qgs_project
                    ),
                    "contactelectronicmailaddress": QgsServerProjectUtils.owsServiceContactMail(
                        qgs_project
                    ),
                }
            )
    ...
    def set_map_themes(self, ret, qgs_project):
        ...
        for map_theme in map_themes:
            theme = {"theme": map_theme, "styles": {}}

            for r in (
                qgs_project.mapThemeCollection().mapThemeState(map_theme).layerRecords()
            ):
                theme["styles"].update({r.layer().id(): r.currentStyle})
    ...
    def get_bookmarks(self, qgs_project):
        ...
        for bgroup in bgroups:
            ...
            if bgroup:
                to_add = {"name": bgroup, "expanded": False, "nodes": []}
    ...
    def get_messages(self, instance):
        ...
        return {
            ...
            "items": [
                {
                    "id": m.pk,
                    "title": getattr(m, title_col)
                    if hasattr(m, title_col)
                    else m.title,
    ...
    def to_representation(self, instance):
        ...
        try:
            initextent_by_geoconstraint = (
                GeoConstraintRule.get_max_extent_on_project_for_user(
            ...
            logger.error(
                f"[Project serializer] Initextent by geocontraint error: {str(e)}"
            )

        ret["print"] = (
            json.loads(clean_for_json(instance.layouts)) if instance.layouts else []
        )
        # Ordering
        ...
        def readLeaf(layer, container):
            ...
            if "nodes" in layer:
                ...
                if (
                    settings.G3W_CLIENT_NOT_SHOW_EMPTY_VECTORLAYER
                    and self.layer_is_empty(layers[layer["id"]])
                ):
                ...
                try:
                    ...
                    logger.error("Layer %s is missing from QGIS project!" % layer["id"])
                    return
                ...
                for signal_receiver, data in after_serialized_project_layer.send(
                    layer_serialized, layer=layers[layer["id"]], request=self.request
                ):
                ...
                if layer_serialized_data["vectorjoins"]:
                    ret["relations"] += self.get_map_layers_relations_from_vectorjoins(
                        layer["id"], layer_serialized_data["vectorjoins"], layers
                    )
        ...
        for l in ret["layerstree"]:
            try:
                ...
                logger.error("Layer %s is missing from QGIS project!" % l["id"])
...
class LayerSerializer(G3WRequestSerializer, serializers.ModelSerializer):
    ...
    def column_to_exclude(self, instance):
        ...
        return (
            eval(instance.exclude_attribute_wms)
            if instance.exclude_attribute_wms
            else []
        )
    ...
    def to_representation(self, instance):
        ...
        if instance.layer_type in [Layer.TYPES.wms, Layer.TYPES.arcgismapserver]:
            ...
            if (
                "username" not in ret["source"] or "password" not in ret["source"]
        ...
        if (
            "showfeaturecount" in self.layertreenode
            and self.layertreenode["showfeaturecount"]
        ):
    ...
    def to_representation(self, instance):
        ...
        if ret["type"] == "search":
            ...
            if "otherlayers" in body:
                ret["options"].update({"otherquerylayerids": body["otherlayers"]})
            ...
            for field in body["fields"]:
                ...
                if "widgettype" in field and field["widgettype"] == "selectbox":
                    ...
                    if field["name"] in edittypes and widget_type in (
                        "ValueMap",
                        "ValueRelation",
                    ):
