...
TYPE_RASTER_LAYER_FOR_DOWNLOAD = ("gdal", "raster")
...
class Project(G3WProjectMixins, G3WACLModelMixins, TimeStampedModel):
    ...
    toc_layers_init_status = models.CharField(
        ...
        help_text=_(
            "Set tab's TOC layers initials state: 'Collapsed (close)'"
    ...
    toc_themes_init_status = models.CharField(
        ...
        default="collapsed",
        help_text=_(
            "Set map themes list initials state: 'Collapsed (close)'"
            "or 'Not collapsed (open)'"
        ),
    )
    ...
    use_map_extent_as_init_extent = models.BooleanField(
        _("User QGIS project map start extent as webgis init extent"), default=False
    )
    ...
    order = models.PositiveIntegerField(
        _("Fields to se order"), default=0, blank=True, null=True
    )
    ...
    def _permissionsToEditor(self, user, mode="add"):
        ...
        if not user.has_perm("core.view_group", self.group):
            setPermissionUserObject(
                user, self.group, permissions=["core.view_group"], mode=mode
            )
    ...
    def _permissions_to_user_groups_editor(self, groups_id, mode="add"):
        ...
        for group_id in groups_id:
            ...
            if "view_group" not in get_perms(auth_group, self.group):
                setPermissionUserObject(
                    auth_group, self.group, permissions=["core.view_group"], mode=mode
                )
    ...
    def url_alias(self):
        try:
            return ProjectMapUrlAlias.objects.get(
                app_name="qdjango", project_id=self.pk
            ).alias
        except:
            return None
    ...
    def url_alias(self, url_alias):
        if url_alias:
            ProjectMapUrlAlias.objects.update_or_create(
                app_name="qdjango", project_id=self.pk, defaults={"alias": url_alias}
            )
        else:
            try:
...
class Layer(G3WACLModelMixins, models.Model):
    ...
    slug = AutoSlugField(_("Slug"), populate_from=["name"], unique=True, overwrite=True)
    is_active = models.BooleanField(_("Is active"), default=1)
    # Project
