...
// -------------------------------------------------
_.extend(g3wadmin.widget, {
  _scalevisconstraintManagerParams: ["scalevisconstraint-list-url", "scalevisconstraint-layer-pk", "info-layer-user", "info-layer-authgroup"],
  _scalevisconstraintDetailUrl: "/" + SITE_PREFIX_URL + "qdjango/api/scalevisconstraint/detail/",
  /**
   * Creates the ScaleLayerVisibilityConstraint table
   */
  _scalevisconstraintTable: function (res, $datatable, $item) {
    var params = ga.utils.getDataAttrs($item, this._scalevisconstraintManagerParams)
    let scalevisconstraint_list_url = params["scalevisconstraint-list-url"]
    if (_.isUndefined(params["scalevisconstraint-layer-pk"])) {
      throw new Error("Attribute scalevisconstraint-layer-pk not defined")
    }

    let $div = $('<div style="margin-left:40px;">')

    // add new btn
    $newscalevisconstraint = $('<a href="#" class="btn btn-sm btn-default"><i class="ion ion-plus-circled"></i> ' + gettext("Create New Scale Visibility Layer Constraint") + "</a>")
    $newscalevisconstraint.on("click", function () {
      ga.widget._newscalevisconstraintForm($datatable, $newscalevisconstraint, params, {
        "modal-title": gettext("New Scale Visibility Layer Constraint"),
        new: true,
        parent_click: $(this),
      })
    })
    $div.append($newscalevisconstraint)
    ...
    $.each(res["results"], function (k, v) {
      ...
      actions += `
            <span class="col-xs-2 icon">
                <a href="#" data-scalevisconstraint-pk="${v["pk"]}"
                    data-toggle="tooltip" data-placement="top"
    ...
    // Add events to actions
    $tbody.find('[data-scalevisconstraint-action-mode="delete"]').on("click", function (e) {
      let scalevisconstraint = $(e.currentTarget).attr("data-scalevisconstraint-pk")
      let actionDelete = function () {
        $.ajax({
          method: "delete",
          url: ga.widget._scalevisconstraintDetailUrl + scalevisconstraint + "/",
          success: function (res) {
            ga.widget.showscalevisconstraintManagerList($datatable, $item, true)
            modal.hide()
          },
          complete: function () {
            // Nothing to do
      ...
      var modal = ga.ui.buildDefaultModal({
        ...
        modalBody: `${gettext("Are you sure to delete constraint")} #<strong>${scalevisconstraint}</strong>?`,
        closeButtonText: "No",
      })
    ...
    $tbody.find('[data-scalevisconstraint-action-mode="edit"]').on("click", function (e) {
      let scalevisconstraint = $(e.currentTarget).attr("data-scalevisconstraint-pk")
      let scalevisconstraint_record = $(e.currentTarget).data("scalevisconstraint-record")
      ga.widget._newscalevisconstraintForm($datatable, $newscalevisconstraint, params, {
        "modal-title": gettext("Edit Scale Visibility Layer Constraint") + " " + scalevisconstraint,
        new: false,
        scalevisconstraint_record: scalevisconstraint_record,
        parent_click: $(this),
      })
    })
  ...
  /**
   * Display the Scale visibility layer list
   */
  showscalevisconstraintManagerList: function ($datatable, $item, refresh) {
    try {
      var params = ga.utils.getDataAttrs($item, this._scalevisconstraintManagerParams)
      if (_.isUndefined(params["scalevisconstraint-list-url"])) {
      ...
      var getDetail = function () {
        $.ajax({
          method: "get",
          url: params["scalevisconstraint-list-url"],
          success: function (res) {
           row.child(g3wadmin.widget._scalevisconstraintTable(res, $datatable, $item)).show()
          },
          complete: function () {
            var status = arguments[1]
            if (status == "success") {
   ...
   *
   */
  _newscalevisconstraintForm: function ($datatable, $item, params, form_params) {
    let isNew = form_params["new"]
    params["layerId"] = params["scalevisconstraint-layer-pk"]

    if (isNew) {
      params["form_action"] = params["scalevisconstraint-list-url"]
      params["title"] = gettext("Define minscale and maxscale for a user or a group.")
      params["modal_title"] = gettext("Create a new Scale Visibility Layer Constraint.")
      params["scalevisconstraintPk"] = ""
    } else {
      params["form_action"] = ga.widget._scalevisconstraintDetailUrl + form_params["scalevisconstraint_record"]["pk"] + "/"
      params["title"] = gettext("Edit Scale Visibility Layer Constraint.")
      params["modal_title"] = params["title"] + "#" + form_params["scalevisconstraint_record"]["pk"]
      params["scalevisconstraintPk"] = form_params["scalevisconstraint_record"]["pk"]
    }

    let modal = (ga.currentModal = ga.ui.buildDefaultModal({
      modalTitle: params["modal_title"],
      modalBody: ga.tpl._newscalevisconstraintForm(params),
      modalSize: _.isUndefined(params["modal-size"]) ? "" : params["modal-size"],
    }))
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        if (!isNew) {
          if (form_params["scalevisconstraint_record"]["user"] != "") {
            $el.val(form_params["scalevisconstraint_record"]["user"])
          }
        }
      },
      complete: function () {},
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        if (!isNew) {
          if (form_params["scalevisconstraint_record"]["group"] != "") {
            $el.val(form_params["scalevisconstraint_record"]["group"])
          }
        }
      },
      complete: function () {},
    ...
    // Populate max and min scale
    if (!isNew) {
      let $elmax = modal.$modal.find('[name="maxscale"]')
      $elmax.val(form_params["scalevisconstraint_record"]["maxscale"])
      let $elmin = modal.$modal.find('[name="minscale"]')
      $elmin.val(form_params["scalevisconstraint_record"]["minscale"])
    }


    // parent_click based on new or update
    if (isNew) {
      var $item = form_params["parent_click"].parents("tr").prev().find('[data-widget-type="scalevisconstraintManagerList"]')
    } else {
      var $item = $(form_params["parent_click"].parents("table")[0]).parents("tr").prev().find('[data-widget-type="scalevisconstraintManagerList"]')
    }
    ...
    modal.setConfirmButtonAction(function (e) {
      ...
      // Maxscale and min scale not empty
      if (dt["minscale"] == "" || dt["maxscale"] == "") {
        let $ediv = form.$form.find(".form-errors")
        $ediv.html("")
        $ediv.append(`<h4 class="badge bg-red">${gettext("The 'maxscale' and/or 'minscale' fields cannot be empty!")}</h4>`)
        return
      }
...
// activate widget: append to ga.ui.before_datatable_callbacks for to cala it before DatTable init
ga.ui.before_datatable_callbacks.push(function ($widgetItem) {
  $widgetItem.find('[data-widget-type="scalevisconstraintManagerList"]').on("click", function (e) {
    var $datatable = $(this).parents("table").DataTable()
    ga.widget.showscalevisconstraintManagerList($datatable, $(this))
  })
})

_.extend(g3wadmin.tpl, {
  _newscalevisconstraintForm: _.template(`
        <form action="<%= form_action %>" id="form-new-scalevisconstraint-<%= layerId %>">
        <input type="hidden" name="pk" value="<%= scalevisconstraintPk %>">
        <input type="hidden" name="layer" value="<%= layerId %>">
        <div class="form-errors"></div>
            <div class="row">
				<div class="col-md-12">
					...
					<div class="form-group">
						...
						<div class="controls ">
							<input class="form-control" value="" required name="minscale" style="width:100%;">
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="controls ">
							<input class="form-control" value="" required name="maxscale" style="width:100%;">
						</div>
					</div>
