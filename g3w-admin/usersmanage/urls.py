...
urlpatterns = [
    ...
    url(r'^users/groups/(?P<pk>[-_\w\d]+)/$',login_required(UserDetailView.as_view()),name='user-detail'),
    url(r'^users/groups/update/(?P<pk>[-_\w\d]+)/$',login_required(UserUpdateView.as_view()),name='user-update'),
    url(r'^users/groups/delete/(?P<pk>[-_\w\d]+)/$',login_required(UserAjaxDeleteView.as_view()), name='user-delete'),
