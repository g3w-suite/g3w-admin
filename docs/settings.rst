...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Custom django modules that i possible to add, i.e. ``g3w-admin-caching`` (https://github.com/g3w-suite/g3w-admin-caching) module and other third part django modules.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
A dict of custom links to add into main top bar of wbgis client.
I.e.::
